...
function activity2() {
   ...
   <div>
    ...
    scene = new Scene();
    exp_setup_img = new Chemistry.Custome_image(forward_bias_setup, new Chemistry.Point(600, 350), 2264 * 0.5, 1272 * 0.5, canvas);
    scene.add(exp_setup_img);
    scene.draw();
    V_dsp = document.getElementById('voltage-dsp');
    I_dsp = document.getElementById('current-dsp');
    forward_bias_toggle_btn = (document.getElementById('forward-bias-toggle'));
    diode_inp = document.getElementById('diode');
...
function add_description_btn() {
    pp.showdescription(`<p class='discription_text'>
      ...
      <p class='discription_text'>
         Note the readings for ${forward_bias ? 'forward bias' : 'reverse bias'} in the observation table and click on verify.
      </p>
      `, 3);
    pp.addtorightpannel(`<button id="panel1_btn" class="btn btn-primary" onclick="verify_reading()" style="position: absolute; bottom: 12vh; width: 91%;">Verify</button>`, 3);
}
function diode_selected(val) {
    V_slider = document.getElementById('voltage');
    V_inp = (document.getElementById(`a2-${forward_bias ? 'fb' : 'rb'}-V-inp-${reading_count + 1}`));
    I_inp = (document.getElementById(`a2-${forward_bias ? 'fb' : 'rb'}-I-inp-${reading_count + 1}`));
    switch (val) {
        case '1N4148':
            I = I_4148;
            break;
...
function onVoltageChange(ele) {
    ...
    if (forward_bias) {
        ...
        V_inp = (document.getElementById(`a2-fb-V-inp-${reading_count + 1}`));
        I_inp = (document.getElementById(`a2-fb-I-inp-${reading_count + 1}`));
    }
    else {
        ...
        V_inp = (document.getElementById(`a2-rb-V-inp-${reading_count + 1}`));
        I_inp = (document.getElementById(`a2-rb-I-inp-${reading_count + 1}`));
    }
    V_inp.disabled = false;
    I_inp.disabled = false;
    add_description_btn();
...
function verify_reading() {
    ...
    if (reading_count === 4) {
        ...
        if (completed_first_reading_set) {
            pp.showdescription(`
               <p class='discription_text'>
                  Successfully taken all the readings for ${forward_bias ? 'forward' : 'reverse'} bias.
               </p>
               <p class='discription_text'>
            ...
            pp.showdescription(`
               ...
               <p class='discription_text'>
                  Now switch to ${forward_bias ? 'reverse' : 'forward'} bias and take the readings.
               </p>
            `, 3);
            show_panel(3);
            forward_bias_toggle_btn.disabled = false;
...
function check_duplicate_data(bias) {
    if (bias) {
