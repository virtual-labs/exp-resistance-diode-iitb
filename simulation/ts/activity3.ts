...
function activity3() {
	...
	pp.addtorightpannel(
      ...
      <button  class="btn btn-primary" onclick="a3_verify()" style="position: absolute; bottom: 10vh; width: 91%;">Verify</button>`,
		3
	);
   ...
   `;
	...
	canvas_div = <HTMLDivElement>document.getElementById('act3-canvas-div');
	data_div = <HTMLDivElement>document.getElementById('act3-data-div');
...
function a3_verify() {
	let v_f_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-v-f-inp')
	);

	let i_f_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-i-f-inp')
	);

	let del_v_f_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-del-v-f-inp')
	);

	let del_i_f_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-del-i-f-inp')
	);

	let v_r_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-v-r-inp')
	);

	let i_r_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('act3-i-r-inp')
	);

	let ratio_div: HTMLDivElement = <HTMLDivElement>(
		document.getElementById('act3-ratio-div')
	);
	...
	ratio_div.innerHTML = `
      $$
         \\begin{aligned}
            \\text{Forward static resistance}(R_F) &= \\frac{V_F}{I_F} = ${parseFloat(
				(V_f / I_f).toFixed(3)
			)} \\ k\\Omega \\\\ \\\\
            \\text{Forward dynamic resistance}(r_F) &= \\frac{\ΔV_F}{\ΔI_F} = ${parseFloat(
				(del_v_f / del_i_f).toFixed(3)
			)} \\ k\Ω \\\\ \\\\
            \\text{Reverse static resistance}(R_R) &= \\frac{V_R}{I_R} = ${parseFloat(
				(V_r / I_r).toFixed(3)
			)} \\ k\\Omega
         \\end{aligned} 
      $$
